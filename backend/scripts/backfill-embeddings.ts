...
import { backfillRiskEmbeddings } from '../src/services/embeddingService';
...
Options:
  --batch-size <number>    Number of risks to process per batch (default: 10)
...
Examples:
  ...
  npm run backfill-embeddings -- --batch-size 20 --concurrency 4
...
async function main() {
  ...
  try {
    const stats = await backfillRiskEmbeddings(batchSize, concurrency, dryRun);
